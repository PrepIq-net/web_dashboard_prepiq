...
const TopNavComponent = memo(function DashboardTopNav() {
  ...
  /**
   * A notification in this dropdown is a shortcut to the screen it is about.
   * Clicking one used to only mark it read, which left you holding the
   * information and no way to act on it.
   */
  const handleOpenNotification = (notification: Notification) => {
    ...
    // No screen answers this one — the feed is where its full text lives.
          ...
          <div className="relative" ref={notificationsRef}>
            ...
            {notificationsOpen ? (
              <div className="absolute right-0 z-30 mt-2 w-[360px] rounded-[12px] border border-[#2E2E33] bg-[#1C1C1F] p-3 shadow-[0_8px_24px_rgba(0,0,0,0.4)]">
                ...
                <div className="max-h-80 space-y-1 overflow-y-auto design-scrollbar pr-1">
                ...
                {/* Outside the list, and outside the "has notifications" branch:
                    the feed carries history and category filters, so a quiet
                    today is no reason to strand you with only a URL to type. */}
                <div className="pt-2">
                  <Link
            ...
            {avatarMenuOpen ? (
              <div className="absolute right-0 z-30 mt-2 w-72 rounded-2xl border border-[#2E2E33] bg-[#1C1C1F] shadow-[0_8px_32px_rgba(0,0,0,0.5)] overflow-hidden">
                ...
                <div className="p-2">
                  ...
                  {/* No notifications entry here — the bell beside this menu is
                      the single entry point, badge and all. Two of them meant
                      two unread counts to keep in agreement. */}
                </div>
