...
export default function POSConnectPage() {
  ...
  async function handleSquareConnect(branchIdOverride?: string) {
    ...
    const postConnectRedirect = `${window.location.origin}/setup/sales/pos/square/connected`;

    try {
      const result = await squareOAuth.mutateAsync({
  ...
  function handleBranchSelect(nextBranchId: string) {
    ...
    if (!nextBranchId || squareOAuth.isPending || scopeLoading || scopeError) {
      return;
    }
    void handleSquareConnect(nextBranchId);
  }
  ...
  return (
    <div className="min-h-screen bg-[#141416] flex items-center justify-center p-6">
      <div className="w-full max-w-lg">
        ...
        <button
          onClick={() => {
            void handleSquareConnect();
          }}
          disabled={squareOAuth.isPending || scopeLoading || !selectedBranchId}
