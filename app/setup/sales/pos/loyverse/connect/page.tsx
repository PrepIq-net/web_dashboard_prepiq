...
export default function LoyverseConnectPage() {
  ...
  async function handleLoyverseConnect(branchIdOverride?: string) {
    ...
    const postConnectRedirect = `${window.location.origin}/setup/sales/pos/loyverse/connected`;
    try {
      const result = await loyverseOAuth.mutateAsync({
  ...
  function handleBranchSelect(nextBranchId: string) {
    ...
    if (
      !nextBranchId ||
      loyverseOAuth.isPending ||
      scopeLoading ||
      scopeError
    ) {
      return;
    }
    void handleLoyverseConnect(nextBranchId);
  }
  ...
  return (
    <div className="min-h-screen bg-[#141416] flex items-center justify-center p-6">
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-[12px] border border-[#2E2E33] bg-[#1C1C1F] p-5 mb-4">
          <div className="flex items-start gap-2.5 mb-4">
            ...
            <p className="text-[13px] leading-[20px] text-[#C7C7CC]">
              You will be redirected to Loyverse and then returned here after
              authorization.
            </p>
          </div>
        ...
        {(scopeError || loyverseOAuth.error) && (
            ...
            <div className="flex items-start gap-2.5">
              ...
              <p className="text-[13px] leading-[20px] text-[#C7C7CC]">
                {scopeError instanceof Error
                  ? scopeError.message
                  : loyverseOAuth.error instanceof Error
                    ? loyverseOAuth.error.message
                    : "Unable to start Loyverse connection."}
              </p>
            </div>
          </div>
        )}
        ...
        <button
          onClick={() => {
            void handleLoyverseConnect();
          }}
          disabled={
            loyverseOAuth.isPending || scopeLoading || !selectedBranchId
          }
