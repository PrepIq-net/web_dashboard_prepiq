...
export function ToastConnectedContent() {
  ...
  const title = isConnected
    ? t("setup.sales.upload.connected.toastSuccessTitle")
    : t("setup.sales.upload.connected.toastFailTitle");

  const message = useMemo(() => {
    if (isConnected) {
      return t("setup.sales.upload.connected.toastSuccessMsg");
    }

    if (errorDescription) return errorDescription;
    if (error) return t("setup.sales.upload.connected.toastError", { error });
    ...
    <div className="w-full max-w-lg rounded-[12px] border border-[#2E2E33] bg-[#1C1C1F] p-6">
      ...
      <div className="flex flex-col gap-3">
        {isConnected ? (
          ...
          <button
            onClick={() => router.push("/setup/sales/pos/toast/connect")}
            className="w-full h-11 rounded-[8px] bg-[#A8821F] hover:bg-[#B8962E] active:bg-[#8F6F18] text-[#141416] text-sm font-semibold inline-flex items-center justify-center gap-2 transition-colors"
          >
            {t("setup.sales.upload.connected.toastRetry")}
