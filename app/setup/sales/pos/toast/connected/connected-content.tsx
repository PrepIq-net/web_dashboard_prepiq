...
export function ToastConnectedContent() {
  ...
  const title = isConnected
    ? "Toast connected"
    : "Toast connection not completed";

  const message = useMemo(() => {
    if (isConnected) {
      return "PrepIQ can now pull sales data from your Toast account. Your restaurants will be monitored automatically.";
    }

    if (errorDescription) return errorDescription;
    if (error) return `Toast returned: ${error}`;
      ...
      <div className="flex items-center gap-2 mb-5">
        ...
        <span
          className={`text-[11px] font-semibold uppercase tracking-[0.18em] ${
            isConnected ? "text-[#3F8F68]" : "text-[#C48B2A]"
          }`}
        >
